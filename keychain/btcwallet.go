...
func (b *BtcWalletKeyRing) DerivePrivKey(keyDesc KeyDescriptor) (
	*btcec.PrivateKey, error) {
	...
	//
	// TODO: the PubKey == nil guard exists because when a descriptor
	// carries a non-nil PubKey we can't tell whether KeyLocator.Index
	// was set explicitly to 0 or just left at the Go zero value. The
	// PubKey-set branch below defensively scans up to MaxKeyRangeScan
	// derivations for a PubKey match rather than trusting the path,
	// so the fast DeriveFromKeyPathCache path is also gated on
	// PubKey == nil. As a result, hot callers like the node identity
	// key (Family=NodeKey, Index=0, PubKey populated by DeriveKey)
	// always miss this cache and take the read-write wallet DB
	// transaction path on every call. Once we have a way to mark a
	// descriptor's path as authoritative (or btcwallet exposes a
	// key-cache lookup by PubKey), the per-ECDH cache in
	// derivePrivKeyForECDH below can be dropped in favor of this path.
...
//
// TODO: this cache only exists because btcwallet's internal
